...
#' @param min.base.qual The minimum base quality needs to be adjusted for long reads (optional, default=15).
#' @param min.map.qual The minimum mapping quality required for it to be counted (optional, default=35).
#' @param additional_flags Additional flags to be passed on to alleleCounter (optional, e.g. -f 0 for long read data or -r genome.fa for CRAN processing)
...
#' @export
ascat.getBAFsAndLogRs = function(samplename, tumourAlleleCountsFile.prefix, normalAlleleCountsFile.prefix, tumourLogR_file, tumourBAF_file, normalLogR_file, normalBAF_file, alleles.prefix, gender, genomeVersion, chrom_names=c(1:22,'X'), minCounts=20, BED_file=NA, probloci_file=NA, seed=as.integer(Sys.time()), tumour_only_mode=F, loci_binsize = 1) {
  set.seed(seed)
  ...
  if (loci_binsize > 1) {
    ...
    unifidx <- sort(x = union(homidx, hetidx), decreasing = F)
...
ascat.prepareHTS = function(tumourseqfile, normalseqfile=NA, tumourname, normalname=NA, allelecounter_exe, alleles.prefix, loci.prefix, gender, genomeVersion,
                            ...
                            skip_allele_counting_tumour=F, skip_allele_counting_normal=F, loci_binsize = 1, seed=as.integer(Sys.time())) {
